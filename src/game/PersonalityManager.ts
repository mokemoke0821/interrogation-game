...
export class PersonalityManager {
  ...
  static generateEmotion(phase: GamePhase, _mentalLife: number): string {
    ...
    return (
      phaseEmotions[Math.floor(Math.random() * phaseEmotions.length)] || '😐'
    );
  }
  ...
  static generatePartialTruth(hiddenTruths: string[]): string {
    ...
    const truth =
      hiddenTruths[Math.floor(Math.random() * hiddenTruths.length)]!;
    ...
    return (
      partialReveals[Math.floor(Math.random() * partialReveals.length)] || ''
    );
  }
   ...
   */
  static generateInnerThought(
    phase: GamePhase,
    _status: { mentalLife: number; alertLevel: number }
  ): string {
    ...
    return (
      phaseThoughts[Math.floor(Math.random() * phaseThoughts.length)] || ''
    );
  }
}
