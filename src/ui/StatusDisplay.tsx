...
const StatusDisplay: React.FC = () => {
  const { suspect, suspectStatus, currentPhase, interrogationPoints } =
    useGameStore();
  ...
  const getPhaseColor = (phase: GamePhase) => {
    switch (phase) {
      case GamePhase.PHASE1_CONFIDENT:
        return 'text-green-400';
      case GamePhase.PHASE2_NERVOUS:
        return 'text-yellow-400';
      case GamePhase.PHASE3_CONFUSED:
        return 'text-orange-400';
      case GamePhase.PHASE4_BREAKDOWN:
        return 'text-red-400';
      default:
  ...
  return (
    <div className='bg-gray-800 rounded-lg p-6 border border-gray-700'>
      ...
      <div className='mb-6'>
        ...
        <p className='text-sm text-red-400 mt-1'>容疑: {suspect.crime}</p>
      </div>

      {/* フェーズ表示 */}
      <div
        className={`text-center mb-6 p-3 rounded-lg bg-gray-900 border ${getPhaseColor(currentPhase)} border-current`}
      >
